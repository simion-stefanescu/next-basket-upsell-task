...
const OrderConfirmation: React.FC<OrderConfirmationProps> = ({
  ...
  products.forEach(
    (p) => (productMap[p.id] = { name: p.name, price: p.price }),
  );
  ...
  return (
    ...
    >
          ...
          {order.items.map((item: any, idx: number) => (
            <tr key={idx}>
              ...
              <td>
                $
                {(
                  (productMap[item.product_id]?.price || 0) * item.quantity
                ).toFixed(2)}
              </td>
            </tr>
          ))}
      ...
      <div style={{ marginBottom: 8 }}>
        <div>
          Subtotal: <b>${order.subtotal.toFixed(2)}</b>
        </div>
        {order.discount > 0 && (
          <div>
            Promo Discount:{" "}
            <b style={{ color: "#16a34a" }}>- ${order.discount.toFixed(2)}</b>
          </div>
        )}
        <div>
          VAT (20%): <b>${order.vat.toFixed(2)}</b>
        </div>
